...
class PriceActionStrategy(bt.Strategy):
    ...
    def _update_market_structure(self) -> None:
        ...
        try:
            ...
            self.log(f"Market Structure: {self.current_trend.value}, "
    ...
    def _detect_zone_retest(self) -> Tuple[Optional[str], Optional[Dict]]:
        ...
        for zone in self.demand_zones:
            ...
            if (self._is_price_in_zone(current_price, zone) or 
                self._is_price_in_zone(current_low, zone)):
                return 'demand', zone
        ...
        for zone in self.supply_zones:
            # Price touching or entering the zone
            if (self._is_price_in_zone(current_price, zone) or 
                self._is_price_in_zone(current_high, zone)):
                return 'supply', zone
    ...
    def _place_long_order(self, zone: Dict) -> None:
        ...
        try:
            ...
            self.log(f"LONG SIGNAL: Entry: {current_price:.2f}, "
    ...
    def _place_short_order(self, zone: Dict) -> None:
        ...
        try:
            ...
            self.log(f"SHORT SIGNAL: Entry: {current_price:.2f}, "
                    f"Stop: {stop_loss:.2f}, Target: {take_profit:.2f}, "
                    f"Size: {position_size:.2f}, Risk: {stop_loss_distance:.2f}")
    ...
    def next(self) -> None:
        ...
        if (len(self.data_buffer) < 20 or 
            len(self.pending_orders) >= self.p.max_concurrent_trades):
            return
        ...
        if zone_type == 'demand':
            self.log(f"DEMAND ZONE RETEST: Zone strength: {zone.get('strength', 0):.2f}, "
                    f"Zone price: {zone['price']:.2f}")
            self._place_long_order(zone)
        elif zone_type == 'supply':
            self.log(f"SUPPLY ZONE RETEST: Zone strength: {zone.get('strength', 0):.2f}, "
                    f"Zone price: {zone['price']:.2f}")
